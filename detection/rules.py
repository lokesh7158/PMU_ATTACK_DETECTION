...
class StreamState:
    ...
    inter_arrival_history: deque = field(default_factory=lambda: deque(maxlen=RULE_HISTORY_WINDOW))
    jitter_history: deque = field(default_factory=lambda: deque(maxlen=RULE_HISTORY_WINDOW))
    timing_variance_history: deque = field(default_factory=lambda: deque(maxlen=RULE_HISTORY_WINDOW))
    replay_fingerprints: deque = field(default_factory=lambda: deque(maxlen=RULE_HISTORY_WINDOW))
    score_history: deque = field(default_factory=lambda: deque(maxlen=RULE_HISTORY_WINDOW))
    voltage_history: deque = field(default_factory=lambda: deque(maxlen=RULE_HISTORY_WINDOW))
    current_history: deque = field(default_factory=lambda: deque(maxlen=RULE_HISTORY_WINDOW))
    freq_history: deque = field(default_factory=lambda: deque(maxlen=RULE_HISTORY_WINDOW))
    angle_history: deque = field(default_factory=lambda: deque(maxlen=RULE_HISTORY_WINDOW))
    anomaly_history: deque = field(default_factory=lambda: deque(maxlen=RULE_HISTORY_WINDOW))
    fps_history: deque = field(default_factory=lambda: deque(maxlen=RULE_HISTORY_WINDOW))
    arrival_times: deque = field(default_factory=lambda: deque(maxlen=RULE_HISTORY_WINDOW))
    byte_events: deque = field(default_factory=lambda: deque(maxlen=RULE_HISTORY_WINDOW))
    frames_per_second: float = 0.0
    packets_per_second: float = 0.0
    bytes_per_second: float = 0.0
    burst_rate: float = 0.0
    rate_violation_count: int = 0
    inter_arrival_violation_count: int = 0
    expected_sequence: Optional[int] = None
    last_sequence: Optional[int] = None
    recent_sequences: deque = field(default_factory=lambda: deque(maxlen=RULE_HISTORY_WINDOW))
    ...
    route_delay_history: deque = field(default_factory=lambda: deque(maxlen=RULE_HISTORY_WINDOW))
    route_latency_anomaly_count: int = 0
    recent_sequence_steps: deque = field(default_factory=lambda: deque(maxlen=RULE_HISTORY_WINDOW))
...
DEFAULT_DETECTOR_THRESHOLDS = {
    ...
    "enable_route_latency_rules": True,
...
def load_thresholds(path: Optional[str] = None) -> Dict[str, Any]:
    ...
    fallback_path = project_root / "packet_reader" / "thresholds.json"
    candidate_paths = []
    if path is not None:
...
def _profile_thresholds(profile: Dict[str, Any], name: str) -> Dict[str, float]:
    ...
    if name == "phase_angle_diff" and _safe_float(stats.get("p95"), 0.0) > 10.0:
        stats = _safe_dict(profile.get("angle_diff", stats))
    merged = {}
...
def _profile_envelope(stats: Dict[str, Any], floor: float, default_mean: Optional[float] = None) -> Tuple[float, float]:
    lower = _safe_float(stats.get("lower_bound"))
    upper = _safe_float(stats.get("upper_bound"))
    mean = _safe_float(stats.get("mean"), default_mean)
    std = _safe_float(stats.get("std"), 0.0)

    if lower is None:
        lower = max((mean or 0.0) - 3.0 * (std or 0.0), floor)
    if upper is None:
        upper = max((mean or floor) + 3.0 * (std or 0.0), floor)
    if upper < lower:
        upper = lower
    return max(lower, 0.0), max(upper, floor)
...
def _window_mean_delta(current: float, history: deque, window: int) -> float:
    values = list(history)[-window:]
    if not values:
        return 0.0
    return abs(current - (sum(values) / len(values)))


def _max_window_mean_delta(current: float, history: deque, windows: Tuple[int, ...] = (5, 10)) -> float:
    return max((_window_mean_delta(current, history, window) for window in windows), default=0.0)
...
def _phase_spacing_error(frame: Dict[str, Any]) -> float:
    ...
    if va is None or vb is None or vc is None:
        return 0.0
    if max(abs(va), abs(vb), abs(vc)) <= (2.0 * math.pi + 0.1):
        va = math.degrees(va)
        vb = math.degrees(vb)
        vc = math.degrees(vc)
...
class FrameRuleEngine:
    ...
    def _build_context(
        ...
        return {
            ...
            "angle_history": list(state.angle_history),
    ...
    def _timing_evaluations(self, frame: Dict[str, Any], state: StreamState, context: Dict[str, Any]) -> List[RuleEvaluation]:
        ...
        profile = context.get("profile", {})
        inter_arrival_stats = _safe_dict(profile.get("inter_arrival", {}))
        timing_source = _safe_str(inter_arrival_stats.get("source")) or "soc_fracsec"
        observed_time = _safe_float(frame.get("frame_time"), _safe_float(frame.get("capture_time"), time.time()))
        previous_observed_time = state.last_observed_capture_time or state.last_capture_time
        if timing_source != "soc_fracsec" and previous_observed_time:
            inter_arrival = max(observed_time - previous_observed_time, 0.0)
        elif state.last_timestamp and current_ts:
        ...
        if inter_arrival is not None:
            ...
            expected_fps = self._expected_fps(profile)
            expected_interval = 1.0 / expected_fps if expected_fps > 0 else None
            min_bound, max_bound = _profile_envelope(inter_arrival_stats, floor=0.001, default_mean=expected_interval)
            ia_rule = None
            if inter_arrival < min_bound:
                ia_rule = RuleEvaluation("HARD_IA_MIN_10", "Minimum inter-arrival bound exceeded", "timing", 90, SeverityLevel.LOW, 0.65, 24.0, f"Inter-arrival {inter_arrival:.6f}s is below the trusted lower bound")
            elif inter_arrival > max_bound:
                ia_rule = RuleEvaluation("HARD_IA_MAX_11", "Maximum inter-arrival bound exceeded", "timing", 90, SeverityLevel.LOW, 0.65, 24.0, f"Inter-arrival {inter_arrival:.6f}s exceeds the trusted upper bound")

            if ia_rule is not None:
                state.inter_arrival_violation_count += 1
                required = max(_safe_int(self._detector_config().get("inter_arrival_consecutive_frames"), 3), 1)
                if state.inter_arrival_violation_count >= required:
                    evaluations.append(ia_rule)
            else:
                state.inter_arrival_violation_count = 0
            ...
            if len(state.inter_arrival_history) >= 4:
                ...
                jitter_threshold = max((max_bound - min_bound) * 3.0, 0.05)
            ...
            silence_multiplier = max(_safe_float(self._detector_config().get("silence_timeout_multiplier"), 5.0), 1.0)
            if state.last_capture_time and capture_time - state.last_capture_time > max_bound * silence_multiplier:
                evaluations.append(RuleEvaluation("HARD_SILENCE_13", "Silence timeout", "timing", 88, SeverityLevel.LOW, 0.65, 24.0, "No PMU frames were observed for longer than the allowed silence window"))
    ...
    def _rate_evaluations(self, frame: Dict[str, Any], state: StreamState, context: Dict[str, Any]) -> List[RuleEvaluation]:
        ...
        burst_samples = [stamp for stamp in state.arrival_times if observed_time - stamp <= burst_window]
        burst_elapsed = observed_time - burst_samples[0] if len(burst_samples) >= 2 else 0.0
        state.burst_rate = max(0.0, (len(burst_samples) - 1) / burst_elapsed) if burst_elapsed > 0 else 0.0
    ...
    def _sequence_evaluations(self, frame: Dict[str, Any], state: StreamState) -> List[RuleEvaluation]:
        ...
        sequence_source = "sequence_number"
        if sequence is None and _safe_bool(self._detector_config().get("use_frame_number_sequence"), True):
            ...
            sequence_source = "frame_number"
        if sequence is None:
            return evaluations

        jump_threshold = max(_safe_int(self._detector_config().get("sequence_jump_threshold"), 1), 1)
        missing_threshold = max(_safe_int(self._detector_config().get("missing_frame_threshold"), 2), 1)
        replay_window = max(_safe_int(self._detector_config().get("sequence_replay_window"), 32), 2)
        profile = self._get_profile(_safe_str(frame.get("pmu_id")))
        sequence_profile = _safe_dict(_safe_dict(profile.get("sequence", {})).get(f"{sequence_source}_step", {}))
        profiled_step = _safe_float(sequence_profile.get("mode"))
        profiled_upper = _safe_float(sequence_profile.get("upper_bound"))
        learned_step = None
        if state.recent_sequence_steps:
            counts = defaultdict(int)
            for step in state.recent_sequence_steps:
                counts[int(round(step))] += 1
            learned_step = max(counts.items(), key=lambda item: item[1])[0]
        expected_step = max(_safe_int(profiled_step, learned_step or 1), 1)
        gap_limit = expected_step * (missing_threshold + 1) + jump_threshold
        if profiled_upper is not None:
            gap_limit = max(gap_limit, profiled_upper)
        ...
        if state.last_sequence is not None:
            if sequence == state.last_sequence:
                ...
                sequence_step = sequence - state.last_sequence
                state.recent_sequence_steps.append(sequence_step)
                if sequence_step > gap_limit:
                    missing = max(int(round(sequence_step / expected_step)) - 1, 1)
                    if missing >= missing_threshold:
                        state.sequence_gap_counter += missing
                        evaluations.append(RuleEvaluation("SEQ_GAP_39", "Missing frames or sequence jump", "timing", 78, SeverityLevel.LOW, 0.70, 24.0, f"Sequence advanced by {sequence_step}; expected step={expected_step}; estimated missing frames={missing}"))
    ...
    def _suppression_evaluations(self, frame: Dict[str, Any], state: StreamState, context: Dict[str, Any]) -> List[RuleEvaluation]:
        ...
        timing_source = _safe_str(_safe_dict(profile.get("inter_arrival", {})).get("source")) or "soc_fracsec"
        if timing_source == "soc_fracsec":
            if current_ts is None:
                state.last_observed_timestamp = current_ts or state.last_observed_timestamp
                return evaluations
            ...
            previous_seconds = state.last_observed_capture_time
            current_seconds = _safe_float(frame.get("frame_time"), _safe_float(frame.get("capture_time"), time.time()))
    ...
    def _mitm_symptom_evaluations(self, frame: Dict[str, Any], state: StreamState, context: Dict[str, Any], current_evaluations: List[RuleEvaluation]) -> List[RuleEvaluation]:
        ...
        latency_enabled = _safe_bool(self._detector_config().get("enable_route_latency_rules"), True)
        if latency_enabled and state.last_observed_capture_time and previous_seconds is not None and current_seconds is not None:
    ...
    def _compute_disturbance(self, frame: Dict[str, Any], state: StreamState, profile: Dict[str, Any]) -> Dict[str, Any]:
        ...
        voltage_window_delta = _max_window_mean_delta(voltage, state.voltage_history)
        current_window_delta = _max_window_mean_delta(current, state.current_history)
        freq_window_delta = _max_window_mean_delta(freq, state.freq_history)
        freq_oscillation = _stddev((list(state.freq_history) + [freq])[-10:])
        angle_oscillation = _stddev((list(state.angle_history) + [phase_angle_error])[-10:])
        ...
        thresholds = {
            ...
            "phase_angle_diff": _effective_threshold(angle_stats, floor=0.5),
        }

        ratios = {
            "voltage": min(1.5, max(voltage_delta, voltage_window_delta) / max(thresholds["voltage"], 1e-6)),
            "current": min(1.5, max(current_delta, current_window_delta) / max(thresholds["current"], 1e-6)),
            "frequency": min(1.5, max(freq_delta, freq_window_delta) / max(thresholds["frequency"], 1e-6)),
            ...
            "freq_oscillation": min(1.5, freq_oscillation / max(thresholds["frequency"] * 0.75, 1e-6)),
            "angle_oscillation": min(1.5, angle_oscillation / max(thresholds["phase_angle_diff"] * 0.75, 1e-6)),
        }

        weighted_terms = [
            (0.16, ratios["voltage"]),
            (0.14, ratios["current"]),
            (0.18, ratios["frequency"]),
            (0.18, ratios["rocof"]),
            (0.10, ratios["smoothness"]),
            (0.06, ratios["voltage_imbalance"]),
            (0.05, ratios["current_imbalance"]),
            (0.15, ratios["phase_angle_diff"]),
            (0.10, ratios["freq_oscillation"]),
            (0.08, ratios["angle_oscillation"]),
        ]
        ...
        return {"score": score, "active": score >= 15.0, "multiplier": multiplier}
    ...
    def _physics_evaluations(self, frame: Dict[str, Any], state: StreamState, context: Dict[str, Any], disturbance_multiplier: float) -> List[RuleEvaluation]:
        ...
        voltage_threshold = _effective_threshold(v_stats, floor=0.01)
        current_threshold = _effective_threshold(c_stats, floor=0.01)
        frequency_threshold = _effective_threshold(f_stats, floor=0.05)
        rocof_threshold = _effective_threshold(r_stats, floor=0.05)
        angle_threshold = _effective_threshold(angle_stats, floor=0.5)
        smooth_threshold = _effective_threshold(s_stats, floor=0.01)
        ...
        if max(voltage_delta, voltage_window_delta) > voltage_threshold and abs(current_delta) < current_threshold:
            tier = self._anomaly_tier(max(voltage_delta, voltage_window_delta), v_stats)
            item = self._profile_rule("PHYS_VOLTAGE_DRIFT_46", "Voltage long-window drift", tier, "Voltage moved away from the recent 5/10/20-frame baseline")
            if item is not None:
                evaluations.append(item)

        if max(current_delta, current_window_delta) > current_threshold and abs(voltage_delta) < voltage_threshold:
            tier = self._anomaly_tier(max(current_delta, current_window_delta), c_stats)
            item = self._profile_rule("PHYS_CURRENT_DRIFT_47", "Current long-window drift", tier, "Current moved away from the recent 5/10/20-frame baseline")
            if item is not None:
                evaluations.append(item)

        if max(freq_delta, freq_window_delta) > frequency_threshold:
            tier = self._anomaly_tier(max(freq_delta, freq_window_delta), f_stats)
            item = self._profile_rule("PHYS_FREQ_DRIFT_48", "Frequency long-window drift", tier, "Frequency moved away from the recent 5/10/20-frame baseline")
            if item is not None:
                evaluations.append(item)

        if len(state.freq_history) >= 2 and abs(freq_delta) > frequency_threshold and rocof > rocof_threshold and abs(current_delta) > current_threshold:
            tier = self._anomaly_tier(abs(freq_delta) + rocof, {"p95": frequency_threshold + rocof_threshold, "p99": frequency_threshold + rocof_threshold + 0.05, "std": 0.05})
        ...
        if len(state.freq_history) >= 10 and freq_oscillation > frequency_threshold * 0.75:
            tier = self._anomaly_tier(freq_oscillation, {"p95": frequency_threshold * 0.75, "p99": frequency_threshold, "std": max(frequency_threshold * 0.25, 0.01)})
            item = self._profile_rule("PHYS_FREQ_OSC_49", "Frequency oscillation", tier, "Frequency variability over the last 10 frames exceeded the trusted baseline")
            if item is not None:
                evaluations.append(item)

        if len(state.angle_history) >= 10 and angle_oscillation > max(angle_threshold * 0.75, 1.0):
            tier = self._anomaly_tier(angle_oscillation, {"p95": angle_threshold * 0.75, "p99": angle_threshold, "std": max(angle_threshold * 0.25, 0.01)})
            item = self._profile_rule("PHYS_ANGLE_OSC_50", "Angle oscillation", tier, "Phase-angle variability over the last 10 frames exceeded the trusted baseline")
            if item is not None:
                evaluations.append(item)
        ...
        if phase_angle_error > max(angle_threshold * 4.0, 2.0):
    ...
    def _compute_fault_score(self, frame: Dict[str, Any], state: StreamState, profile: Dict[str, Any], disturbance_score: float) -> float:
        ...
        fault_signals = []
        if max(voltage_delta, voltage_window_delta) >= voltage_threshold:
            fault_signals.append(30)
        if max(current_delta, current_window_delta) >= current_threshold:
            fault_signals.append(30)
        if max(freq_delta, freq_window_delta) >= frequency_threshold:
            fault_signals.append(26)
        if rocof >= rocof_threshold:
            fault_signals.append(26)
        if phase_angle_error >= angle_threshold:
            fault_signals.append(22)
        if len(state.freq_history) >= 10 and freq_oscillation >= frequency_threshold * 0.75:
            fault_signals.append(22)
        if len(state.angle_history) >= 10 and angle_oscillation >= angle_threshold * 0.75:
            fault_signals.append(18)
        ...
        return min(100.0, max(base_fault, disturbance_score * 0.85))
    ...
    def _safe_update(self, frame: Dict[str, Any], state: StreamState, fused: Dict[str, Any], evaluations: List[RuleEvaluation], disturbance_score: float, anomaly_flag: bool) -> None:
        if fused["final_score"] >= self.suspicious_threshold or any(item.severity in {SeverityLevel.HIGH, SeverityLevel.CRITICAL} and item.category == "structural" for item in evaluations):
            ...
            state.voltage_history.append(_voltage_mean(frame))
            state.current_history.append(_current_mean(frame))
            state.freq_history.append(_safe_float(frame.get("freq1"), 50.0))
            state.angle_history.append(_phase_spacing_error(frame))
    ...
    def evaluate_frame(self, frame: Dict[str, Any], history_frames: Optional[pd.DataFrame] = None) -> Dict[str, Any]:
        ...
        disturbance_raw_score = disturbance["score"]
        disturbance_active = disturbance["active"]
        disturbance_score = disturbance_raw_score if disturbance_active else 0.0
        effective_disturbance_score = disturbance_score if disturbance_active else disturbance_raw_score * 0.7
        ...
        fault_score = self._compute_fault_score(normalized, state, context.get("profile", {}), effective_disturbance_score)
